...
In this section, an introduction to satellite missions, sensors and products available with RADAR-API is provided.
...
The mission relies on the TRMM, the :ref:`GPM Core Observatory (GPM-CO) satellite <radar_api_core_satellite>` and a constellation of satellites equipped
with microwave radiometers which forms the so-called :ref:`GPM Constellation <radar_api_constellation>`.
...
- :ref:`GPM Dual-frequency Precipitation Radar (DPR) <radar_api_dpr>`
- :ref:`GPM Microwave Imager (GMI) <radar_api_gmi>`

The :ref:`GPM Data Archive <radar_api_data_archive>` currently includes satellite data records that extend back to 1987.
...
The data are organized into various :ref:` product levels <radar_api_product_levels>`,
...
The `Tropical Rainfall Measuring Mission (TRMM) satellite <https://radar_api.nasa.gov/missions/trmm>`_,
...
the `Visible and Infrared Scanner (VIRS) imager <https://radar_api.nasa.gov/missions/TRMM/satellite/virs>`_,
...
:ref:`GPM constellation <radar_api_constellation>` polar-orbiting sensors which typically acquire observations at fixed local times.
...
The TRMM `Precipitation Radar (PR) <https://radar_api.nasa.gov/missions/TRMM/satellite/PR>`_ is a Ku-band (13.6 GHz) cross-track scanning radar with a swath coverage of 215 km.
...
The `TRMM Microwave Imager (TMI) <https://radar_api.nasa.gov/missions/TRMM/satellite/TMI>`_ is a passive microwave, dual-polarization conically scanning radiometer equipped with 9 channels.
...
.. _radar_api_core_satellite:

GPM Core Observatory
---------------------

The `GPM Core Observatory (GPM-CO) satellite <https://radar_api.nasa.gov/missions/GPM/core-observatory>`_,
...
Equipped with advanced instruments such as the :ref:`Dual-frequency Precipitation Radar (DPR) <radar_api_dpr>` and the
:ref:`GPM Microwave Imager (GMI) <radar_api_gmi>`, the GPM Core Observatory can accurately measure a wide range of precipitation types,
...
The `GPM Dual-frequency Precipitation Radar (DPR) <https://radar_api.nasa.gov/missions/GPM/DPR>`_ features a Ka-band precipitation radar (KaPR)
...
The `GPM Microwave Imager (GMI) <https://radar_api.nasa.gov/missions/GPM/GMI>`_ is a dual-polarization, multi-channel, conical- scanning, microwave radiometer
...
.. figure:: https://radar_api.nasa.gov/sites/default/files/2019-11/GMI-13-channels.jpg
...
.. _radar_api_constellation:

GPM Constellation
--------------------

The `GPM Constellation <https://radar_api.nasa.gov/missions/GPM/constellation>`_ is composed by satellites from various space agencies, each equipped with microwave radiometers.
...
.. _radar_api_data_archive:

GPM Data Archive
-------------------

RADAR-API provides tools to easily search files on the *Precipitation Processing System* (PPS)
...
- GES DISC GPM Data: `<https://radar_api1.gesdisc.eosdis.nasa.gov/data>`_
...
The RADAR-API does not currently support methods for searching, downloading, and opening products from JAXA and FengYun data centers,
but contributions to expand the RADAR-API to include these data centers are very welcome !

.. _radar_api_product_levels:
...
Currently, the RADAR-API provide access to the IMERG products and all L1 and L2 GPM products.
L3 products are currently not available via RADAR-API, but can be manually computed using the
Geographic Bucket Toolbox provided by the software.

You can retrieve the list of products available through the RADAR-API using the ``radar_api.available_products()`` function.
For a comprehensive online list of GPM products, refer to `this page <https://radar_api.nasa.gov/data/directory>`_
...
Currently, RADAR-API offers access to versions 5, 6, and 7. Version 7 is the latest and is recommended for most applications.
...
`GPM Products File Specification <https://radar_api.nasa.gov/resources/documents/file-specification-radar_api-products>`_,
...
Use ``radar_api.available_products(category="RADAR", product_levels="2A")`` to list the available products.
...
Brightness temperature measurements from the :ref:`PMW sensors <pmw>` within the :ref:`GPM constellation <radar_api_constellation>` are available in the L1B and L1C products.
...
Use ``radar_api.available_products(category="PMW", product_levels="1C")`` to list the available products.
...
refer to the `DPR ATBD <https://radar_api.nasa.gov/resources/documents/radar_api-dpr-level-2-algorithm-theoretical-basis-document-atbd>`_.
...
The Goddard Profiling Algorithm (GPROF) is used to retrieve precipitation estimates from all PMW sensors of the :ref:`GPM constellation <radar_api_constellation>`.
...
refer to the `GPROF ATBD <https://radar_api.nasa.gov/resources/documents/radar_api-gprof-algorithm-theoretical-basis-document-atbd>`_.

Precipitation estimates derived from GPM Constellation sensors are available in the L2A products.
Use ``radar_api.available_products(category="PMW", product_levels="2A")`` to list the available products.
...
The TRMM/GPM CORRA product can be accessed with RADAR-API using the ``2B-TRMM-CORRA`` and ``2B-GPM-CORRA``  product acronyms.

For the theoretical and mathematical details of the combined radar/radiometer algorithm please
refer to the `CORRA ATBD <https://radar_api.nasa.gov/resources/documents/radar_api-combined-radar-radiometer-precipitation-algorithm-theoretical-basis>`_.
...
By leveraging measurements from Geostationary (GEO) IR imagers and the :ref:`GPM PMW sensors constellation <radar_api_constellation>`,
...
`IMERG version 6 <https://radar_api.nasa.gov/resources/documents/algorithm-information/IMERG-V06-ATBD>`_ and
`IMERG version 7 <https://radar_api.nasa.gov/sites/default/files/2023-07/IMERG_V07_ATBD_final_230712.pdf>`_.
...
Two algorithms, the `Goddard Convective-Stratiform Heating (CSH) <https://radar_api.nasa.gov/resources/documents/goddard-convective-stratiform-heating-csh-algorithm>`_ and the
...
The TRMM/GPM SLH and CSH products can be accessed with RADAR-API using the ``2A-TRMM-SLH``, ``2B-TRMM-CSH``, ``2A-GPM-SLH``, ``2B-GPM-CSH`` product acronyms.
...
None of the following datasets is currently supported by RADAR-API but we welcome contributions to expand the software capabilities.
...
and the `Algorithm Theoretical Basis Document (ATBD) <https://radar_api.nasa.gov/resources/documents/cloudsat-radar_api-coincidence-dataset-version-1c>`_.

The coincidence dataset can be downloaded with RADAR-API using the ``2B-GPM-CSAT`` and ``2B-TRMM-CSAT`` product acronyms.
...
This dataset will expand the lightning record from the `TRMM-LIS <https://radar_api.nasa.gov/data/directory/trmm-lis>`_ into the GPM era.
...
While these datasets are not currently supported by the RADAR-API, we welcome contributions to expand the software capabilities.
...
Data are available for `TRMM here <http://trmm.atmos.washington.edu/>`_  and `GPM here <http://radar_api.atmos.washington.edu/>`_.
...
.. code-block:: python
    ...
    p = ds["precipRateNearSurface"].radar_api.plot_map()
...
allows you to visualize the :ref:`GPM PMW constellation <radar_api_constellation>` swath coverage over a 1-hour period.
...
+============+=================================================================================+=====================+====================+==================================================================================================+
| IMERG      | Integrated Multi-satellitE Retrievals for GPM                                   | 30 minutes          | 0.1°               | `NASA <https://radar_api.nasa.gov/data/imerg>`_                                                        |
+------------+---------------------------------------------------------------------------------+---------------------+--------------------+--------------------------------------------------------------------------------------------------+
...
RADAR-API currently provides only access to the version 6 and 7 of the IMERG products, and the variable ``IRprecipitation`` in such IMERG products results from the PERSIANN-CCS and PDIR-Now algorithms respectively.

We welcome contributions that enable RADAR-API to access other precipitation products !
...
the `GPM IMERG Global Viewer <https://radar_api.nasa.gov/data/visualization/global-viewer>`_ and
...
.. list-table::
   ...
   * - Training
     ...
     - | `NASA Materials <https://radar_api.nasa.gov/data/training>`_
   ...
   * - GPM News
     ...
     - | `JAXA GPM News <https://www.eorc.jaxa.jp/GPM/en/index.html>`_
       | `NASA GPM News <https://radar_api.nasa.gov/data/news>`_
   * - GPM FAQ
     - Frequently Asked Questions about GPM data
     - `GPM FAQ <https://radar_api.nasa.gov/data/faq>`_
   * - GPM Mission
     - Global Precipitation Measurement Mission
     - | `NASA GPM <https://radar_api.nasa.gov/missions>`_
       | `JAXA GPM <https://www.eorc.jaxa.jp/GPM/en/index.html>`_
       | `eoPortal GPM <https://www.eoportal.org/satellite-missions/radar_api>`_
   * - TRMM Mission
   ...
   * - GPM Products Handbook
     ...
     - | `GPM Data Handbook <https://www.eorc.jaxa.jp/GPM/doc/data_utilization/GPM_data_util_handbook_E.pdf>`_
       | `GPM File Specifications <https://radar_api.nasa.gov/resources/documents/file-specification-radar_api-products>`_
   * - ATBDs
     - Algorithm Theoretical Basis Documents
     - | `GPM Documents <https://radar_api.nasa.gov/resources/documents>`_
